...
void generateSubsequences(string ip, string op, vector<string>& result) {
    // Base Case: If the input string is empty, add the output string to the result
    if (ip.empty()) {
...
// 		    //as choices are 2 , whether to  include ip or not.
    ...
    // Remove the first character from the input string and recurse for the remaining string
    ip.erase(ip.begin());
