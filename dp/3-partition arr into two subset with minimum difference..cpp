...
subset2 sum = totalsum - subset1 sum
...
class Solution {
  public:
  int t[101][10001]; // Memoization table (adjust size based on constraints)- bottom up
...
int minDifference(vector<int>& arr) {
    ...
    //i need to create a 2d matrix that consist of 2 parameters that are changing in recursive solution and initialise all cells as false
    ...
    //this dp table will store that dp[i][j], represent  a subproblem possible  from arr  with i elem in arr and j as sum of subset
    ...
    //table fill ho gyi ab arr size = n wali row ye bta rhi h ki jab arr m total n elem hai tab konse subsetsum possible h us ko iterate kro 
    ...
    int mindiff = INT_MAX;
  //subset2sum is rem sum totalsum-subsetsum1 or  (totalsumsum - 2*subsetsum1) is expression ki min val tab milegi jab subsetsum1 will be  totalsum/2 
  //isliye subsetsum1 totalsum/2 se kam ya uske pass hi hona chahiye
    for(int subsetsum1 = sumtotal/2; subsetsum1 >=0 ; subsetsum1--){
